...
class NewsFeedGenerator:
    ...
    def start(self):
        ...
        while True:
            ...
            print("4. Download from an XML file")
            print("5. Exit")
    ...
    def import_from_xml(self):
        ...
        while True:
            ...
            if file_choice == "1":
                file_path = importer.get_file_from_default_folder()
                if file_path:
                    new_sections = importer.parse_xml(file_path)
                    if new_sections:
                        self.write_to_file(new_sections)
                        importer.remove_file(file_path)
                break
...
class News:
    ...
    def add_entry(self):
        ...
        db_handler.insert_record(
            "News",
            (capitalize_sentence(news_text.strip()), capitalize_sentence(city.strip()), publish_date)
        )
...
class PrivateAd:
    ...
    def add_entry(self):
        ...
        db_handler.insert_record(
            "PrivateAd",
            (capitalize_sentence(ad_text.strip()), expiration_date, days_left)
        )
...
class BirthAnnouncement:
    ...
    def add_entry(self):
        ...
        db_handler.insert_record(
            "BirthAnnouncement",
            (
                capitalize_sentence(child_name.strip()),
                birth_date,
                capitalize_sentence(location.strip()),
                capitalize_sentence(message.strip())
            )
        )
        ...
        try:
            ...
            for entry in entries:
                ...
                if "news" in header.lower():
                    ...
                    # Save to database
                    db_handler.insert_record("News",
                                             (capitalize_sentence(news_text), capitalize_sentence(city), publish_date))
                    ...
                    # Save to database
                    db_handler.insert_record("PrivateAd", (capitalize_sentence(ad_text), expiration_date, days_left))
                ...
                # Save to database
                    db_handler.insert_record("BirthAnnouncement",
                    (capitalize_sentence(child_name), birth_date, capitalize_sentence(location), capitalize_sentence(message))
                    )
            ...
            for key, item in data.items():
                try:
                    ...
                    if publication_type == "news":
                        ...
                        sections["News"].append(formatted_entry)
                        # Save to database
                        db_handler.insert_record("News", (text, city, publish_date))
                        ...
                        sections["PrivateAd"].append(formatted_entry)
                        # Save to database
                        db_handler.insert_record("PrivateAd", (text, expiration_date, days_left))
                        ...
                        sections["BirthAnnouncement"].append(formatted_entry)
                        # Save to database
                        db_handler.insert_record("BirthAnnouncement", (child_name, birth_date, location, message))
...
# The class to handle importing entries from an XML file
class ImportFromXML:
    def __init__(self, default_folder="imports"):
        self.default_folder = default_folder
        if not os.path.exists(self.default_folder):
    ...
    def get_file_from_default_folder(self):
        files = [f for f in os.listdir(self.default_folder) if f.endswith(".xml")]
        if not files:
            print("No XML files found in the default folder.")
            return None
        return os.path.join(self.default_folder, files[0])
    ...
    def get_file_from_user_path(self):
        ...
        if not os.path.isfile(file_path) or not file_path.endswith(".xml"):
            print("Invalid file path or format. Please provide a valid XML file.")
            return None
        return file_path
    ...
    def parse_xml(self, file_path):
        ...
        try:
            ...
            for publication in root.findall("publication"):
                try:
                    publication_type = publication.find("publication_type").text.strip().lower()

                    if publication_type == "news":
                        text = publication.find("text")
                        city = publication.find("city")

                        # Validate required fields
                        if text is None or city is None or not text.text.strip() or not city.text.strip():
                            print(f"Invalid News entry due to missing fields: {ET.tostring(publication)}")
                            errors_detected = True
                            continue

                        text = capitalize_sentence(text.text.strip())
                        city = capitalize_proper_noun(city.text.strip())
                        publish_date = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
                        ...
                        text = publication.find("text")
                        expiration_date = publication.find("date")

                        # Validate required fields
                        if text is None or expiration_date is None or not text.text.strip() or not expiration_date.text.strip():
                            print(f"Invalid Private Ad entry due to missing fields: {ET.tostring(publication)}")
                            errors_detected = True
                            continue
                        ...
                        try:
                            ...
                            print(
                                f"Invalid date format in Private Ad entry: {expiration_date}. Expected format: DD/MM/YYYY")
                            errors_detected = True
                            continue
                        ...
                        child_name = publication.find("child_name")
                        birth_date = publication.find("birth_date")
                        location = publication.find("location")
                        message = publication.find("message")

                        # Validate required fields
                        if (
                                child_name is None or birth_date is None or location is None or message is None or
                                not child_name.text.strip() or not birth_date.text.strip() or
                                not location.text.strip() or not message.text.strip()
                        ):
                            print(f"Invalid Birth Announcement entry due to missing fields: {ET.tostring(publication)}")
                            errors_detected = True
                            continue

                        child_name = capitalize_proper_noun(child_name.text.strip())
                        birth_date = birth_date.text.strip()
                        location = capitalize_proper_noun(location.text.strip())
                        message = capitalize_sentence(message.text.strip())

                        try:
                            birth_date_obj = datetime.datetime.strptime(birth_date, "%d/%m/%Y")
                            if birth_date_obj > datetime.datetime.now():
                                print(f"Entry skipped due to future birth date: {birth_date}")
                                errors_detected = True
                                continue
                        except ValueError:
                            print(
                                f"Invalid date format in Birth Announcement entry: {birth_date}. Expected format: DD/MM/YYYY")
                            errors_detected = True
                            continue
                        ...
                        print(f"Unknown publication type in entry. Skipping: {ET.tostring(publication)}")
                        errors_detected = True
            ...
            print(f"Failed to parse XML file {file_path}. XML is malformed: {e}")
            return {}
        except Exception as e:
            print(f"Unexpected error occurred while processing XML file: {e}")
            return {}

        # If any errors were detected, warn the user and reject the file
        if errors_detected:
            print(f"Errors detected in XML file '{file_path}'. The file has been rejected.")
            return {}
...
class DatabaseHandler:
    def __init__(self, db_name="news_feed.db"):
        ...
        self.connection = sqlite3.connect(self.db_name)  # Use sqlite3.connect for SQLite databases
        self.create_tables()
            ...
            try:
                ...
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS News (
                        ...
                        UNIQUE(text, city) -- Prevent duplicates
                    );
                """)

                # Create Private Ads table
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS PrivateAd (
                        ...
                        UNIQUE(text, expiration_date) -- Prevent duplicates
                    );
                """)

                # Create Birth Announcements table
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS BirthAnnouncement (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        child_name TEXT NOT NULL,
                        birth_date TEXT NOT NULL,
                        location TEXT NOT NULL,
                        message TEXT NOT NULL,
                        UNIQUE(child_name, birth_date, location) -- Prevent duplicates
                    );
                """)
    ...
    def insert_record(self, table_name, record_data):
        ...
        try:
            if table_name == "News":
                cursor.execute(
                    "INSERT OR IGNORE INTO News (text, city, publish_date) VALUES (?, ?, ?)",
                    record_data
                )
            elif table_name == "PrivateAd":
                cursor.execute(
                    "INSERT OR IGNORE INTO PrivateAd (text, expiration_date, days_left) VALUES (?, ?, ?)",
                    record_data
                )
            elif table_name == "BirthAnnouncement":
                cursor.execute(
                    "INSERT OR IGNORE INTO BirthAnnouncement (child_name, birth_date, location, message) VALUES (?, ?, ?, ?)",
                    record_data
                )
            self.connection.commit()
            print(f"Record successfully saved to {table_name} table.")
        except Exception as e:
            print(f"Failed to insert record into {table_name}: {e}")

# Global DatabaseHandler instance to minimize changes
